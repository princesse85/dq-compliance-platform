...
class LegalTextGenerator:
    ...
    def generate_text(self, category: str, risk_level: str, template_variation: bool = True) -> str:
        """
    ...
    def generate_dataset(
        ...
        if category_distribution is None:
            category_distribution = {"contracts": 0.3, "compliance": 0.25, "litigation": 0.25, "regulatory": 0.2}

        if risk_distribution is None:
            risk_distribution = {"HighRisk": 0.33, "MediumRisk": 0.34, "LowRisk": 0.33}
        ...
        for category in categories:
            ...
            for i, risk_level in enumerate(risk_levels):
                ...
                for _ in range(risk_samples):
                    ...
                    data.append({"text": text, "label": risk_level, "category": category})
    ...
    def create_train_valid_test_splits(
        self, df: pd.DataFrame, train_size: int = 840, valid_size: int = 180, test_size: int = 180
    ...
    def save_splits(self, splits: Dict[str, pd.DataFrame], output_dir: str = "data/text_corpus") -> None:
        """
    ...
    def generate_and_save_dataset(
        ...
        # Create splits
        splits = self.create_train_valid_test_splits(df, train_size, valid_size, test_size)
...
def generate_enhanced_dataset(
    output_dir: str = "data/text_corpus", total_samples: int = 1200, random_seed: int = 42
) -> Dict[str, pd.DataFrame]:
    """
