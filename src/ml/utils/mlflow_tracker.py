...
class MLflowTracker:
    ...
    def start_run(self, run_name: str = None) -> bool:
        ...
        try:
            mlflow.start_run(
                experiment_id=self.experiment_id, run_name=run_name or f"run_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            self.log_parameters(
                {
                    ...
                    "class_distribution": json.dumps(dataset_info.get("class_distribution", {}), cls=NumpyEncoder),
                }
            )
            return True
        except Exception as e:
            ...
            for key, value in report.items():
                if isinstance(value, dict):
                    serializable_report[key] = {
                        k: float(v) if isinstance(v, (int, float, np.number)) else v for k, v in value.items()
                    }
                else:
                    serializable_report[key] = value

            self.log_parameters({"classification_report": json.dumps(serializable_report, cls=NumpyEncoder)})
            return True
        except Exception as e:
    ...
    def log_experiment_summary(self, summary: Dict[str, Any]) -> bool:
        ...
        """
        try:
            self.log_parameters({"experiment_summary": json.dumps(summary, cls=NumpyEncoder)})
            return True
        except Exception as e:
...
def create_mlflow_tracker(
    experiment_name: str = None, tracking_uri: str = None, artifact_location: str = None
) -> MLflowTracker:
    """
